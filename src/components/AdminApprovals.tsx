...
import { CheckCircle, XCircle, Users, Clock, AlertCircle, User, UserCheck, Workflow, Eye } from "lucide-react";
...
const AdminApprovals = ({ user }: { user: any }) => {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      switch (viewMode) {
        case "pending":
          response = await apiClient.getPendingExpenses();
          break;
        case "approved":
          response = await apiClient.getExpenses();
          if (response.data) {
            response.data = response.data.filter((exp: any) => exp.status === "approved");
          }
          break;
        case "rejected":
          response = await apiClient.getExpenses();
          if (response.data) {
            response.data = response.data.filter((exp: any) => exp.status === "rejected");
          }
          break;
        default:
          response = await apiClient.getExpenses();
      ...
      if (response.data) {
        ...
        const approvalPromises = response.data.map(async (expense: any) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold">Admin Approvals</h2>
        <p className="text-muted-foreground">Manage all expense submissions and approvals</p>
      </div>
