...
const ApprovalWorkflow = ({ user, userRole }: { user: any; userRole: string }) => {
  ...
  const handleApproval = async (expenseId: string, approvalId: string, status: "approved" | "rejected") => {
    setLoading(true);
    try {
      const response = await apiClient.updateExpenseApproval(approvalId, status, comments);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold">Approval Workflow</h2>
        <p className="text-muted-foreground">Review and approve expense submissions through multi-level workflow</p>
      </div>
